...
public abstract class AbstractRepairProblem extends Problem {
	...
	protected Integer mp1;
	protected Integer mp2;
	...
	public AbstractRepairProblem(Map<String, Object> parameters) throws Exception {
		...
		mp1 = (Integer) parameters.get("mp1");
		mp2 = (Integer) parameters.get("mp2");
	...
	void invokeASTRequestor() throws IOException {
		...
		if(oneEditDegreeMode) {
			if(mp1 == null) {
				throw new RuntimeException("mp1 not provided as a parameter"); 
			}
		
			System.out.println("modification point:" + mp1);
			
			ModificationPoint mpTmp=modificationPoints.get(mp1);
			System.out.println("mp1:" + mpTmp.getLCNode().getClassName() + ": " + mpTmp.getLCNode().getLineNumber());
			
			modificationPoints = new ArrayList<ModificationPoint>();
			modificationPoints.add(mpTmp);
		
		}			
		else if(twoEditDegreeMode) {
			if(mp1 == null || mp2 == null) {
				throw new RuntimeException("mp1 or mp2 not provided as a parameter"); 
			}
		
			System.out.println("modification points:" + mp1 + ", " + mp2);

			ModificationPoint mpTmp=modificationPoints.get(mp1);
			System.out.println("mp1:" + mpTmp.getLCNode().getClassName() + ": " + mpTmp.getLCNode().getLineNumber());
	
			ModificationPoint mpTmp2=modificationPoints.get(mp2);
			System.out.println("mp2:" +  mpTmp2.getLCNode().getClassName() + ": " + mpTmp2.getLCNode().getLineNumber());
			
			modificationPoints = new ArrayList<ModificationPoint>();
			modificationPoints.add(mpTmp);
			modificationPoints.add(mpTmp2);
	...
	void invokeIngredientScreener() throws JMException {
		...
		if(oneEditDegreeMode) {
			System.out.println("#ingredients:" + modificationPoints.get(0).getIngredients().size());
		}

		int indexMp=0;
		for (ModificationPoint mp1 : modificationPoints) {
			System.out.println("Ingredients for MP"+indexMp++);
			int indexSt=0;
			for(Statement stat: mp1.getIngredients()) {
