...
public class GZoltarFaultLocalizer2 implements IFaultLocalizer {
	...
	public GZoltarFaultLocalizer2(String gzoltarDataDir) throws IOException {
		...
		File testFile = new File(gzoltarDataDir, "tests.csv");
...
/*		File spectraFile = new File(gzoltarDataDir, "spectra.csv");
		...
		File spectraFile = new File(gzoltarDataDir, "ochiai.ranking.csv");
		List<String> fLines = FileUtils.readLines(spectraFile, "UTF-8");
		for (int i = 1; i < fLines.size(); i++) {
			String line = fLines.get(i).trim();
			...
			int startIndex = 0;
			int endIndex = line.indexOf('#');
			String classNameWithDollar = line.substring(startIndex,  endIndex);
			
			int index=classNameWithDollar.indexOf("$");
			String className=classNameWithDollar.substring(0, index) 
		              + "." 
		              + classNameWithDollar.substring(index + 1); 
